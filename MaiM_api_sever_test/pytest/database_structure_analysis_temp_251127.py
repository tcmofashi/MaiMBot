    ...
    def generate_report(self, database_info: Dict[str, Any]) -> str:
        ...
        for table_name, table_info in database_info["tables"].items():
            ...
            for column in table_info["columns"]:
                ...
                report.append(f"| `{column['name']}` | `{column['type']}` | {pk_mark} | {notnull_mark} | `{default_value}` |")
            report.append("")
            ...
            if table_info["foreign_keys"]:
                ...
                for fk in table_info["foreign_keys"]:
                    ...
                    if fk["on_update"] or fk["on_delete"]:
                        actions = []
                        if fk["on_update"]: actions.append(f"ON UPDATE {fk['on_update']}")
                        if fk["on_delete"]: actions.append(f"ON DELETE {fk['on_delete']}")
