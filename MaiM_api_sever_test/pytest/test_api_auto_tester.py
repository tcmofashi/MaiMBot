...
def perform_ws_test(
    url: str, headers: Dict[str, str], message: Optional[str] = None, timeout: int = 10
...
def perform_auto_tests(
    config: Dict[str, Any], timeout: int = 15, fail_fast: bool = False, skip_check: bool = False
) -> int:
    ...
    if auth_cfg.get("login_path"):
        ...
        try:
            print("\n[AUTH] 尝试登录获取Token...")
            status, _, data = perform_request(
                auth_base_url,
                auth_method,
                auth_cfg["login_path"],
                auth_headers,
                auth_cfg.get("params", {}) or {},
                auth_body,
                timeout=timeout,
            )
