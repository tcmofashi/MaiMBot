...
from src.common.database.database_model import AgentApiKeys, create_agent_api_key, validate_agent_api_key, TenantUsers
...
async def create_api_key(request: ApiKeyCreateRequest, user_id: str = Depends(verify_auth_token)):
    """
    为指定的租户和智能体创建新的API密钥
    ...
    try:
        ...
        if not tenant_user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"租户 {request.tenant_id} 不存在")
        ...
        if existing_key:
            logger.warning(
                f"用户标识符 {request.user_identifier} 已被租户 {request.tenant_id} 的智能体 {request.agent_id} 使用"
            )
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail=f"用户标识符 {request.user_identifier} 已被使用"
            )
        ...
        logger.info(
            f"创建API密钥成功: {api_key_record.api_key} (租户: {request.tenant_id}, 智能体: {request.agent_id})"
        )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"创建API密钥失败: {str(e)}")
...
async def list_api_keys(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取API密钥列表失败: {str(e)}")


@router.delete("/api-keys/{api_key_id}", summary="删除API密钥")
async def delete_api_key(api_key_id: int, user_id: str = Depends(verify_auth_token)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"禁用API密钥失败: {str(e)}")
...
async def validate_api_key(request: ApiKeyValidationRequest):
    ...
    try:
        ...
        if result["valid"]:
            logger.info(
                f"API密钥验证成功: {request.api_key[:20]}... -> 租户: {result['tenant_id']}, 智能体: {result['agent_id']}"
            )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"验证API密钥失败: {str(e)}")
