"""
隔离化的心流聊天处理类
支持T+A+C+P四维完全隔离：租户(T) + 智能体(A) + 聊天流(C) + 平台(P)
"""
...
logger = get_logger("isolated_hfc")


class IsolatedHeartFChatting:
    """
    隔离化的心流聊天处理类
    管理一个租户+智能体+聊天流+平台维度的连续Focus Chat循环
    """
    ...
    def __init__(self, chat_id: str, isolation_context: IsolationContext):
        ...
        Args:
            chat_id: 聊天流唯一标识符
            isolation_context: 隔离上下文，包含T+A+C+P维度信息
        """
        # 基础隔离属性
        ...
        # 获取隔离化的聊天流
        self._init_chat_stream()

        # 日志前缀，包含隔离信息
        self.log_prefix = f"[隔离-{self.tenant_id}-{self.agent_id}][{getattr(self.chat_stream, 'platform', 'unknown')}:{getattr(self.chat_stream, 'stream_name', self.stream_id)}]"

        # 初始化隔离化的组件
        self._init_isolated_components()
        ...
        logger.info(f"{self.log_prefix} 隔离化心流聊天初始化完成")
    ...
    def _init_chat_stream(self):
        ...
        if not self.chat_stream:
            raise ValueError(f"无法找到隔离化的聊天流: {self.stream_id}")

    def _init_isolated_components(self):
        """初始化隔离化的组件"""
        # 表情学习器（带隔离上下文）
        ...
        # 动作管理器（可以全局共享，但执行时需要隔离上下文）
        self.action_manager = ActionManager()

        # 动作规划器（带隔离上下文）
        self.action_planner = ActionPlanner(
            chat_id=self.stream_id, action_manager=self.action_manager, isolation_context=self.isolation_context
        )

        # 动作修改器（带隔离上下文）
        self.action_modifier = ActionModifier(
            action_manager=self.action_manager, chat_id=self.stream_id, isolation_context=self.isolation_context
        )

    @property
    def config(self):
        """返回当前聊天流的有效配置（支持隔离上下文）"""
        try:
            # 优先使用隔离化配置
    ...
    async def start(self):
        """启动隔离化的主循环"""
        if self.running:
            logger.debug(f"{self.log_prefix} 隔离化心流聊天已激活，无需重复启动")
            return
        ...
        try:
            ...
            logger.info(f"{self.log_prefix} 隔离化心流聊天启动完成")

        except Exception as e:
            # 启动失败时重置状态
            self.running = False
            self._loop_task = None
            logger.error(f"{self.log_prefix} 隔离化心流聊天启动失败: {e}")
            raise
    ...
    def _handle_loop_completion(self, task: asyncio.Task):
        ...
        try:
            if exception := task.exception():
                logger.error(f"{self.log_prefix} 隔离化心流聊天: 脱离了聊天(异常): {exception}")
                logger.error(traceback.format_exc())
            else:
                logger.info(f"{self.log_prefix} 隔离化心流聊天: 脱离了聊天 (外部停止)")
        except asyncio.CancelledError:
            logger.info(f"{self.log_prefix} 隔离化心流聊天: 结束了聊天")
        ...
        if (
            ...
            if random.random() < question_probability:
                try:
                    ...
                    self.last_active_time = time.time()
                    logger.info(f"{self.log_prefix} 长久没有回复，开始生成问题")
    ...
    def _calculate_question_probability(self) -> float:
        """计算主动发言概率（可以基于隔离配置调整）"""
    ...
    async def _observe(
        ...
        async with global_prompt_manager.async_message_scope(self.chat_stream.context.get_template_name()):
            ...
            # TODO: 使用隔离化的记忆系统
            asyncio.create_task(self._build_isolated_memory_content())
            asyncio.create_task(
            ...
            cfg = self.config
            message_list_before_now = self._get_isolated_messages_before_now(cfg.chat.max_context_size)
            ...
            # 事件处理（使用隔离上下文）
            continue_flag, modified_message = await self._handle_isolated_events(
                EventType.ON_PLAN, None, prompt_info[0], None
            )
    ...
    async def _build_isolated_memory_content(self):
        """构建隔离化的记忆内容"""
        # TODO: 使用隔离化的记忆系统
        # 目前暂时使用全局记忆系统
    ...
    def _get_isolated_messages_before_now(self, max_context_size: int):
        """获取隔离化的历史消息"""
        # TODO: 使用隔离化的消息查询
        # 目前暂时使用全局查询
        from src.chat.utils.chat_message_builder import get_raw_msg_before_timestamp_with_chat

        return get_raw_msg_before_timestamp_with_chat(
            chat_id=self.stream_id,
            timestamp=time.time(),
            limit=int(max_context_size * 0.6),
        )

    async def _handle_isolated_events(self, event_type: EventType, data, prompt: str, chat_id: str):
        """处理隔离化的事件"""
        # TODO: 使用隔离化的事件系统
        # 目前暂时使用全局事件系统
        return await events_manager.handle_mai_events(
            event_type, data, prompt, None, chat_id or self.chat_stream.stream_id
        )
        ...
        try:
            with Timer(f"动作{action_planner_info.action_type}", cycle_timers):
                ...
                if action_planner_info.action_type == "no_reply":
                    return await self._handle_no_reply(action_planner_info, thinking_id)
                elif action_planner_info.action_type == "no_reply_until_call":
                    return await self._handle_no_reply_until_call(action_planner_info, thinking_id)
                elif action_planner_info.action_type == "reply":
                    return await self._handle_reply(
                        action_planner_info, chosen_action_plan_infos, thinking_id, available_actions, cycle_timers
                    )
                else:
                    # 执行普通动作
    ...
    async def _handle_no_reply(self, action_planner_info, thinking_id: str):
        """处理no_reply动作"""
    ...
    async def _handle_no_reply_until_call(self, action_planner_info, thinking_id: str):
        """处理no_reply_until_call动作"""
    ...
    async def _lift_question_reply(self, question: str, question_context: str, thinking_id: str):
        ...
        new_msg = self._get_isolated_messages_before_now(1)
    ...
    async def _main_chat_loop(self):
        ...
        try:
            ...
            # 设置了关闭标志位后被取消是正常流程
            logger.info(f"{self.log_prefix} 隔离化麦麦已关闭聊天")
        except Exception:
            logger.error(f"{self.log_prefix} 隔离化麦麦聊天意外错误，将于3s后尝试重新启动")
            print(traceback.format_exc())
    ...
    async def cleanup(self):
        ...
        try:
            ...
            logger.info(f"{self.log_prefix} 隔离化心流聊天清理完成")
        except Exception as e:
            logger.error(f"{self.log_prefix} 清理隔离化心流聊天时出错: {e}")
