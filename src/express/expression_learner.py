...
class ExpressionLearnerManager:
    ...
    def get_isolated_expression_learner(self, chat_id: str, isolation_context=None) -> ExpressionLearner:
        """获取隔离化的表情学习器"""
        # 暂时返回原有学习器，后续可以扩展为IsolatedExpressionLearner
        # 现在只是为了兼容隔离化心流的接口
        return self.get_expression_learner(chat_id)
