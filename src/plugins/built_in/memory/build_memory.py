...
class GetMemoryTool(BaseTool):
    ...
    parameters = [
        ...
        (
            ...
            "需要获取记忆的时间范围，格式为YYYY-MM-DD HH:MM:SS - YYYY-MM-DD HH:MM:SS",
            False,
            None,
        ),
    ]
    ...
    async def execute(self, function_args: dict[str, Any]) -> dict[str, Any]:
        ...
        # 原任务：从记忆仓库获取答案
        memory_task = asyncio.create_task(global_memory_chest.get_answer_by_question(question=question))
        tasks.append(("memory", memory_task))

        # 新任务：从聊天记录获取答案（如果指定了时间参数）
        chat_task = None
        if has_time_params:
            chat_task = asyncio.create_task(self._get_answer_from_chat_history(question, time_point, time_range))
        ...
        try:
            ...
            if time_point:
                ...
                messages_before = get_messages_by_time_in_chat(
                    chat_id=self.chat_id, start_time=0, end_time=target_timestamp, limit=25, limit_mode="latest"
                )
            ...
            try:
                llm_request = LLMRequest(
                    model_set=model_config.model_task_config.utils_small, request_type="chat_history_analysis"
                )
...
答案："""
                ...
                response, (reasoning, model_name, tool_calls) = await llm_request.generate_response_async(
                    prompt=analysis_prompt, temperature=0.3, max_tokens=256
                )
...
class GetMemoryAction(BaseAction):
    ...
    action_description = "在记忆中搜寻某个问题的答案"
