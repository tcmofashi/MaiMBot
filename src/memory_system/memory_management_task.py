...
class MemoryManagementTask(AsyncTask):
    ...
    async def run(self):
        ...
        try:
            ...
            related_titles, related_contents = await global_memory_chest.choose_merge_target(
                selected_title, selected_chat_id
            )
            ...
            logger.info(
                f"{selected_chat_id} 为 [{selected_title}] 找到 {len(related_contents)} 条相关记忆:{related_titles}"
            )
